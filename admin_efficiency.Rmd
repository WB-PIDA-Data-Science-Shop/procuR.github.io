...
p_count
```
After the standardization process, three main types of non-missing procedures are present on Uruguay’s procurement platform: open procedures, restricted procedures, and direct awards.
```{r}
...
subm_proc_facet_q <- ggplot(tender_periods,
  ...
  geom_text_repel(
    ...
    segment.color      = "blue",
    box.padding        = 0.5,
    direction          = "x",       # 👈 allow horizontal movement to avoid overlap
    max.overlaps       = Inf        # don't drop labels
  ) +
  facet_wrap(~ tender_proceduretype, scales = "free_y") +
  scale_y_continuous(
    expand = expansion(mult = c(0, 0.25))
    # 👆 adds 25% extra space above the tallest bar in each facet
  ) +
...
buyer_long <- ggplot(long_share_buyer_proc_long,
  ...
  labs(
    ...
    y = "Share of tenders (100%)",
    fill = NULL,
    title = "Long tender decision periods (≥ 56 days)\n(calculated in number of contracts)"
  ) +
  theme_minimal(base_size = 14) +
  theme(
    axis.text.x = element_text(angle = 45, hjust = 1),
